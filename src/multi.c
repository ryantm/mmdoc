...
int mmdoc_multi_page(char *toc_path, AnchorLocationArray anchor_locations,
  ...
  fputs("      <main>\n", page_file);
  mmdoc_render_part(anchor_location->file_path, page_file, RENDER_TYPE_MULTIPAGE,
...
int mmdoc_multi(char *out, char *src, char *toc_path, AnchorLocationArray toc_anchor_locations,
  ...
  for (int i = 0; i < toc_anchor_locations.used; i++) {
    ...
    mmdoc_multi_page(toc_path, anchor_locations, search_index_file, anchor_location, prev_anchor_location, next_anchor_location);
